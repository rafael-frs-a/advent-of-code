...
Another change in this solution was replacing `Node` references in the structs with their indices in the nodes vector. This fixed the "cannot borrow `*self` as mutable more than once at a time" error in the `union` method, which resulted from the use of lifetimes with the previous approach.
